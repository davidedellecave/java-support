...
public class LRange {
	...
	public LRange(LocalDateTime lbound, LocalDateTime ubound) {
		this.lower = lbound.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
		this.upper = ubound.atZone(ZoneId.systemDefault()).toInstant().toEpochMilli();
		normalizeBound();
	}
	...
	/**
	 * Create range from value where lowerB substracting perc, and upperB adding
	 * perc
	 * 
	 * @param pivot
	 * @param lboundPerc
	...
	public static LRange create(long pivot, double lboundPerc, double uboundPerc) {
		...
		long ubound = (long) (pivot + pivot * uboundPerc / 100);
		return new LRange(lbound, ubound);
	}
...
//    }
	...
	/// <summary>
	/// Return connected if the distance is less or equals threshold
	/// </summary>
	/// <param name="range"></param>
	/// <param name="thresholdDistance"></param>
	/// <returns></returns>
	public boolean isConnected(LRange range) {
		return isConnected(range, threshold);
	}
	...
	public List<LRange> complement(LRange mask) {
		...
		if (mask.lower < cutRange.lower) {
			r1 = LRange.create(mask.lower, cutRange.lower - threshold);
		}
		if (mask.upper > cutRange.upper) {
			r2 = LRange.create(cutRange.upper + threshold, mask.upper);
		}
		if (r1.isNotEmpty())
			result.add(r1);
		if (r2.isNotEmpty())
	...
	public List<LRange> difference(LRange range) {
		...
		if (isValidBoundAndNotEmpty(this.lower, intersection.lower - threshold)) {
			result.add(new LRange(this.lower, intersection.lower - threshold));
		}
		if (isValidBoundAndNotEmpty(intersection.upper + threshold, this.upper)) {
			result.add(new LRange(intersection.upper + threshold, this.upper));
		}
		return result;
	}
	...
	public String toDateString() {
		...
		String s = start.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
		String e = end.format(DateTimeFormatter.ofPattern("yyyy-MM-dd HH:mm"));
		return "[(" + s + ")-(" + e + ")]";
	}
